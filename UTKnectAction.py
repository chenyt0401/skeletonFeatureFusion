...
filepath = './data/UTK3D/modelBest/60/weights.best.hdf5'  # 只保存最佳模型
...
data = pd.read_pickle('./data/UTK3D/60/orgin_data')
...
import model
model_name = "Resnet18"
model = model.resNet18p.ResNet18(60, 10, shape[2])

# model = model.resNet50.ResNet50([3, 4, 6, 3])

# model = model.inceptionNet10.Inception10(num_blocks=2, num_classes=10)

# model = model.inceptionResmnet.InceptionResNet_V2((60, 147, 1), 10)

# model = model.leNet.LeNet_5(classes=10)

# model = model.alexNet8.Alexnet8(classes=10)

# model = model.VGG16.VGG16Net(classes=10)
...
# 记录数据（训练过程的）
resultPath = './data/UTK3D/60/data.txt'
