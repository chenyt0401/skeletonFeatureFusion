...
# region 当前帧所有关节点相对位移
def JJD(new_data, joints):  # 数据  关节数
    data_x = pd.DataFrame()
    data_y = pd.DataFrame()
    data_z = pd.DataFrame()
    ...
    for i in range(int(joints*(joints-1)/2)):  # 计算
        tem_data = pd.concat([tmp_list_x.iloc[:, i], tmp_list_y.iloc[:, i], tmp_list_z.iloc[:, i]], axis=1)
        tem_data = tem_data.astype(float)
        ret = np.linalg.norm(tem_data.values, ord=None, axis=1)  # 对每一个组合好后的xyz进行开方并取根号（计算距离）
        ret = pd.DataFrame(ret)
        get_data = pd.concat([get_data, ret], axis=1)
    # print(get_data.shape)
    get_data = get_data.T.reset_index(drop=True).T  # 整理索引 sort_index不会改变索引 reset_index索引重置
    # get_data.to_csv('JJD_data.txt', sep='\t', header=True)  # 数据量大 先保存
    get_data = pd.concat([new_data.iloc[:, :4], get_data], axis=1, ignore_index=True)
