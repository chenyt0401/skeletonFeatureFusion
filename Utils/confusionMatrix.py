...
def plot_confusion_matrix(cm,
    ...
    plt.savefig('/Flo_3D.png',dpi=350)
    plt.show()


# 显示混淆矩阵
def plot_confuse(model, x_val, y_val):
    labels = [1, 2, 3, 4, 5, 6, 7, 8]  # 不同数据集需要调整
    ...
    conf_mat = confusion_matrix(y_true=truelabel, y_pred=predictions)  # 得到混淆矩阵数据
    print(conf_mat)
    sns.set(font_scale=2)  # 设置字体大小
    sns.heatmap(conf_mat)  # 设置字体大小
    plt.figure()
